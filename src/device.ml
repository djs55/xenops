...
let add_async ~xs ~hvm x domid =
	...
	*)
	...
	let _ = match x.protocol with
	| None -> ()
	| Some protocol -> Hashtbl.add front_tbl "protocol" (string_of_protocol protocol) in
...
let start ?statefile ~xs ?ip domid =
	...
	let ip = match ip with None -> "127.0.0.1" | Some x -> x in
...
let cmdline_of_disp info =
	...
	let disp_options, wait_for_port =
		...
		| VNC (disp_intf, ip_addr_opt, auto, port, keymap) ->
			let ip_addr = match ip_addr_opt with None -> "127.0.0.1" | Some x -> x in
...
let cmdline_of_info info restore domid =
	...
	[
		...
		"-boot"; info.boot;
	] @ (match info.serial with None -> [] | Some x -> [ "-serial"; x ]) @ [
	...
	@ (match info.monitor with None -> [] | Some x -> [ "-monitor"; x ])
	@ (match info.parallel with None -> [] | Some x -> [ "-parallel"; x ])
